...
__version__ = '0.3'
__date__ = '2013-05-17' # FIXME
__copyright__ = 'Copyright (c), 2011, 2012, 2013, Djalil CHAFAÏ'
...
class W2M():
    ...
    def __init__(self, **kwargs):
        ...
        """
        ## Handling arguments (all have a default value, see below) except
        # the starting url (example http://kab.wikipedia.org/wiki).
        ...
        self.maxu = kwargs.get('maxu',100000000)
    ...
    def show(self):
        ...
        if self.vertices == 0:
            logging.error('abnormal number of vertices. we stop')
        ...
        now = time.strftime('%Y-%m-%d %H:%M',time.localtime())
        print >> fmatrix, "# Created: w2m.py %s from %s" % (now,self.start)
    ...
    def _url_get(self,url): 
        ...
        try:
            result = urllib2.urlopen(request)
            ## old code using only headers to get encoding 
            ## fails if encoding is in meta tag for instance
            #encoding = result.headers.getparam('charset')
            #data = result.read().decode(encoding)
            ##
            ## new code using the chardet module to get encoding
    ...
    def _stamp(self,message):
        ...
        """
        now = time.strftime('%Y-%m-%d %H:%M',time.localtime())
        logging.info('%s %s %s',message,now,self.start)
    # end: W2M._stamp()
# end: W2M
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=argparse.SUPPRESS,
        help='Maximum number of analyzed URLs (default is 100000000')
    parser.add_argument(
        '--maxt', 
        metavar='maxt',
        type=int,
